...
export default function Page() {
  ...
  const handleDownloadInvoice = async (subscriptionId, invoiceNumber) => {
    ...
    try {
      const response = await publisherApi.get(`/api/publisher/subscription/invoice/${subscriptionId}`, { responseType: "blob" });
  ...
  if (loading) return (
    <div style={{ padding: "2rem", color: "#888", fontSize: "1.2rem" }}>
      Loading profile…
    </div>
  );
  if (!profile) return (
    <div style={{ padding: "2rem", color: "#ef4444", fontSize: "1.2rem" }}>
      Profile not found.
    </div>
  );
  ...
  const PLAN_LABELS = { "3m": "3 Months", "6m": "6 Months", "9m": "9 Months", "12m": "12 Months" };
            ...
            {editMode ? (
              <form onSubmit={handleSubmit}>
                <div className="pp-edit-form">
                  ...
                  <div className="pp-upload-row">
                    <img
                      src={profileurl?.url || "https://via.placeholder.com/60"}
                      alt="avatar"
                      className="pp-avatar-sm"
              ...
              <>
                ...
                <div className="pp-avatar-row">
                  ...
                  <div>
                    <p
                      className="pp-avatar-name"
                      style={{ fontSize: "1.35rem", fontWeight: 700 }}
                    >
                      {formData.name || "—"}
                    </p>
                    <p
                      className="pp-avatar-sub"
                      style={{ fontSize: "1.2rem", color: "#6b7280" }}
                    >
                ...
                <div className="pp-info-grid">
                  {orgFields.map(([label, value]) => (
                    <div className="pp-info-cell" key={label}>
                      <p
                        className="pp-info-label"
                        style={{ fontSize: "1.0rem", fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.05em" }}
                      >
                        {label}
                      </p>
                      <p
                        className="pp-info-value"
                        style={{ fontSize: "1.2rem", color: "#1e293b", fontWeight: 500 }}
                      >
                        {value || "—"}
                      </p>
                    </div>
        ...
        <div className="pp-sidebar">
          <div className="pp-mini-card">
            ...
            <div className="pp-sub-row">
              ...
              <span
                className={`pp-badge ${isSubActive ? "pp-badge--active" : "pp-badge--expired"}`}
            ...
            <div className="pp-sub-row">
              ...
              <span className="pp-sub-value" style={{ fontSize: "1.2rem", fontWeight: 600 }}>
                {subscriptionExpiry}
              </span>
            </div>
            ...
            <div className="pp-sub-row">
              ...
              <span
                className={`pp-sub-value ${daysLeft <= 30 ? "pp-sub-value--warn" : ""}`}
      ...
      <div className="pp-card pp-card--full mt-3">
        <div className="pp-table-header">
          <div>
            <p
              className="pp-card-title"
              style={{ fontSize: "1.35rem", fontWeight: 700, marginBottom: 2 }}
            >
              Subscription History
            </p>
            <p
              className="pp-table-sub"
              style={{ fontSize: "1.2rem", color: "#6b7280" }}
            >
        ...
        {subsLoading ? (
          ...
          <div className="pp-empty">
            ...
            <p style={{ fontSize: "1.2rem" }}>No subscription history found.</p>
          </div>
        ) : (
            ...
            <table className="pp-table" style={{ fontSize: "1.2rem" }}>
              <thead>
                <tr>
                  {["#", "Plan", "Price", "Start Date", "Expiry Date", "Payment Ref", "Status", "Invoice"].map(h => (
                    <th
                      key={h}
                      style={{ fontSize: "1.05rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tbody>
                {subscriptions.map((sub, index) => {
                  ...
                  const invNumber = `INV-${String(sub._id).slice(-8).toUpperCase()}`;
                  ...
                  return (
                    <tr key={sub._id}>
                      ...
                      <td className="pp-td-muted" style={{ fontSize: "1.2rem" }}>
                        {fmtDate(sub.startDate || sub.createdAt)}
                      </td>
                      ...
                      <td>
                        <button
                          ...
                          onClick={() => handleDownloadInvoice(sub._id, invNumber)}
